...
def email_failed(subject=None, recipients='jchamber@childrensnational.org;tha@childrensnational.org;kmckinley@childrensnational.org'):
...
def email_results_html(df, subject, date_range, recipients, attachments=[],mail_body='Data were abstracted at 06:00'):
    """
...
def email_info(subject=None, date_range=None, recipients="jchamber@childrensnational.org", content=None):
    ...
    mail.HTMLbody = (f"""{content} 
                        """)
    mail.Send()


def email_simple(subject=None, date_range=None, recipients="jchamber@childrensnational.org", directory=None, content=None):
    """Email with link to data"""
    import win32com.client as win32
    outlook = win32.Dispatch('outlook.application')
    mail = outlook.CreateItem(0)
    mail = outlook.CreateItem(0)
    mail.To = f"{recipients}"
    mail.Subject = subject
    mail.HTMLbody = (f"""You have {content} data available in <br><br>
                        <a href='{directory}'>
                        {directory}</a>
                        """)
    mail.Send()
