...
def query_data():
    ...
    sql = f"""
        ...
        ; with patients as
                ...
                WHERE tat.CHECKIN_DATE_TIME < tat2.CHECKIN_DATE_TIME AND tat2.REASON_FOR_VISIT Not Like 'Wound check' 
                ...
                AND tat2.checkin_date_time between @start and @end
        ...
        left outer join return_note on patients.return_fin = return_note.pt_fin
    ...
    apps_fellows.loc[condition_first_res, ['first_resident_seen', 'first_resident_email', 'first_resident_seen_id']].values
    ...
    condition_first_md = apps_fellows['role_first_md'].isin(['Physician Assistant', 'Nurse Practitioner','Fellow'])
    apps_fellows.loc[condition_first_md, ['provider', 'provider_email', 'provider_id']] = \
    apps_fellows.loc[condition_first_md, ['first_md_seen', 'first_md_email', 'first_md_seen_id']].values
   ...
   """
    ...
    apps_fellows.loc[condition_last_md, ['provider','provider_email', 'provider_id']
                                                ...
                                                ,'last_assigned_md_id']].values
    # need to link to email addresses

    returns_with_admission = return_visits[return_visits['admit_visit2']==1]
    # returns_with_admission['provider']=returns_with_admission['last_assigned_md']
    returns_with_admission.loc[:, 'provider'] = returns_with_admission['last_assigned_md']
    returns_with_admission.loc[:, 'provider_email'] = returns_with_admission['last_assigned_md_email']
    returns_with_admission.loc[:, 'provider_id'] = returns_with_admission['last_assigned_md_id']
